...
/** Exact ISO timestamp (§1): format regex first, then a real epoch value. */
...
export function validateBoardData(data) {
  ...
  const tail =
    checkString(data.nextDecision, "$.data.nextDecision", 300) ??
    checkString(data.sourceLabel, "$.data.sourceLabel");
  ...
  if (data.updatedBy !== "claude" && data.updatedBy !== "human") {
    return { ok: false, message: "invalid field $.data.updatedBy" };
  }
